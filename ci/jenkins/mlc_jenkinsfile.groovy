...
stage('Unittest') {
  ...
  'IR Infra': { node('JUNRU-GPU') { ws(per_exec_ws('tvm/unittest/ir-infra')) {
    init_git()
    unpack_lib("cuda", tvm_lib)
    sh (
      script: "${docker_run} --cpus 1 ${ci_cuda} ./tests/scripts/unity/task_python_relax.sh ir-infra",
      label: "IR Infra",
    )
  }}},

  'Runtime': { node('JUNRU-GPU') { ws(per_exec_ws('tvm/unittest/runtime')) {
    init_git()
    unpack_lib("cuda", tvm_lib)
    sh (
      script: "${docker_run} --cpus 1 ${ci_cuda} ./tests/scripts/unity/task_python_relax.sh runtime",
      label: "Runtime",
    )
  }}},

  'Target': { node('JUNRU-GPU') { ws(per_exec_ws('tvm/unittest/target')) {
    init_git()
    unpack_lib("cuda", tvm_lib)
    sh (
      script: "${docker_run} --cpus 1 ${ci_cuda} ./tests/scripts/unity/task_python_relax.sh target",
      label: "Target",
    )
  }}},
