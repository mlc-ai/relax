...
def test_op_correctness():
    ...
    assert relax.op.transpose(x).op == Op.get("relax.transpose")
...
def test_transpose_infer_struct_info():
    bb = relax.BlockBuilder()
    ...
    _check_inference(
        bb, relax.op.transpose(x0, [2, 3, 1, 0]), relax.TensorStructInfo((3, 4, 2, 1), "float32")
    )
    _check_inference(
        bb, relax.op.transpose(x0, axes=None), relax.TensorStructInfo((4, 3, 2, 1), "float32")
    )
    _check_inference(
        bb, relax.op.transpose(x0, [-2, -3, 3, -4]), relax.TensorStructInfo((3, 2, 4, 1), "float32")
    )
    _check_inference(
        bb, relax.op.transpose(x1, [2, 3, 1, 0]), relax.TensorStructInfo(dtype="float32", ndim=4)
    )
    _check_inference(
        bb, relax.op.transpose(x1, axes=None), relax.TensorStructInfo(dtype="float32", ndim=4)
    )
    _check_inference(bb, relax.op.transpose(x2, axes=None), relax.TensorStructInfo(dtype="float32"))
    _check_inference(
        bb, relax.op.transpose(x3, [2, 3, 1, 0]), relax.TensorStructInfo((3, 4, 2, 1), dtype="")
    )
    _check_inference(
        bb, relax.op.transpose(x3, axes=None), relax.TensorStructInfo((4, 3, 2, 1), dtype="")
    )
    _check_inference(
        bb, relax.op.transpose(x3, [-2, -3, 3, -4]), relax.TensorStructInfo((3, 2, 4, 1), dtype="")
    )
    _check_inference(
        bb, relax.op.transpose(x4, [2, 3, 1, 0]), relax.TensorStructInfo(dtype="", ndim=4)
    )
    _check_inference(
        bb, relax.op.transpose(x4, axes=None), relax.TensorStructInfo(dtype="", ndim=4)
    )
    _check_inference(bb, relax.op.transpose(x5, axes=None), relax.TensorStructInfo(dtype=""))


def test_transpose_infer_struct_info_shape_symbolic():
    bb = relax.BlockBuilder()
    a = tir.Var("a", "int64")
    b = tir.Var("b", "int64")
    c = tir.Var("c", "int64")
    ...
    _check_inference(
        bb, relax.op.transpose(x, [2, 3, 1, 0]), relax.TensorStructInfo((c, d, b, a), "float32")
    )
    _check_inference(
        bb, relax.op.transpose(x, axes=None), relax.TensorStructInfo((d, c, b, a), "float32")
    )
    _check_inference(
        bb, relax.op.transpose(x, [-2, -3, 3, -4]), relax.TensorStructInfo((c, b, d, a), "float32")
    )


def test_transpose_infer_struct_info_shape_var():
    bb = relax.BlockBuilder()
    ...
    _check_inference(
        bb, relax.op.transpose(x0, [0, 1, 2, 3]), relax.TensorStructInfo(s0, "float32")
    )
    _check_inference(
        bb, relax.op.transpose(x0, [-4, -3, -2, -1]), relax.TensorStructInfo(s0, "float32")
    )
    _check_inference(
        bb, relax.op.transpose(x0, [2, 3, 0, 1]), relax.TensorStructInfo(dtype="float32", ndim=4)
    )
    _check_inference(
        bb, relax.op.transpose(x0, axes=None), relax.TensorStructInfo(dtype="float32", ndim=4)
    )
    _check_inference(
        bb, relax.op.transpose(x1, [0, 1, 2, 3]), relax.TensorStructInfo(s1, "float32")
    )
    _check_inference(
        bb, relax.op.transpose(x1, [2, 3, 0, 1]), relax.TensorStructInfo(dtype="float32", ndim=4)
    )
    _check_inference(
        bb, relax.op.transpose(x1, axes=None), relax.TensorStructInfo(dtype="float32", ndim=4)
    )
...
def test_transpose_infer_struct_info_more_input_dtype():
    bb = relax.BlockBuilder()
    ...
    _check_inference(
        bb, relax.op.transpose(x0, [2, 3, 1, 0]), relax.TensorStructInfo((3, 4, 2, 1), "float16")
    )
    _check_inference(
        bb, relax.op.transpose(x1, [2, 3, 1, 0]), relax.TensorStructInfo((3, 4, 2, 1), "int8")
    )
    _check_inference(
        bb, relax.op.transpose(x2, [2, 3, 1, 0]), relax.TensorStructInfo((3, 4, 2, 1), "int32")
    )


def test_transpose_infer_struct_info_unknown_ndim_with_axes():
    bb = relax.BlockBuilder()
    ...
    with pytest.raises(TVMError):
        bb.normalize(relax.op.transpose(x0, [2, 3, 1, 0]))
    with pytest.raises(TVMError):
        bb.normalize(relax.op.transpose(x1, [2, 3, 1, 0]))


def test_transpose_infer_struct_info_wrong_number_axes():
    bb = relax.BlockBuilder()
    s0 = relax.Var("s", relax.ShapeStructInfo((1, 2, 3, 4)))
    s1 = relax.Var("s", relax.ShapeStructInfo(ndim=4))
    x0 = relax.Var("x", R.Tensor((1, 2, 3, 4), "float32"))
    ...
    with pytest.raises(TVMError):
        bb.normalize(relax.op.transpose(x0, [0, 2, 1]))
    with pytest.raises(TVMError):
        bb.normalize(relax.op.transpose(x0, [1, 2, 4, 0, 3]))
    with pytest.raises(TVMError):
        bb.normalize(relax.op.transpose(x1, [0, 2, 1]))
    with pytest.raises(TVMError):
        bb.normalize(relax.op.transpose(x1, [1, 2, 4, 0, 3]))
    with pytest.raises(TVMError):
        bb.normalize(relax.op.transpose(x2, [0, 2, 1]))
    with pytest.raises(TVMError):
        bb.normalize(relax.op.transpose(x2, [1, 2, 4, 0, 3]))
    with pytest.raises(TVMError):
        bb.normalize(relax.op.transpose(x3, [0, 2, 1]))
    with pytest.raises(TVMError):
        bb.normalize(relax.op.transpose(x3, [1, 2, 4, 0, 3]))


def test_transpose_infer_struct_info_axis_out_of_range():
    bb = relax.BlockBuilder()
    x0 = relax.Var("x", R.Tensor((1, 2, 3, 4), "float32"))
    x1 = relax.Var("x", R.Tensor("float32", ndim=4))

    with pytest.raises(TVMError):
        bb.normalize(relax.op.transpose(x0, [0, 3, 4, 1]))
    with pytest.raises(TVMError):
        bb.normalize(relax.op.transpose(x0, [0, -5, 1, 3]))
    with pytest.raises(TVMError):
        bb.normalize(relax.op.transpose(x1, [0, 3, 4, 1]))
    with pytest.raises(TVMError):
        bb.normalize(relax.op.transpose(x1, [0, -5, 1, 3]))


def test_transpose_infer_struct_info_repetitive_axes():
    bb = relax.BlockBuilder()
    x0 = relax.Var("x", R.Tensor((1, 2, 3, 4), "float32"))
    x1 = relax.Var("x", R.Tensor("float32", ndim=4))

    with pytest.raises(TVMError):
        bb.normalize(relax.op.transpose(x0, [0, 2, 2, 1]))
    with pytest.raises(TVMError):
        bb.normalize(relax.op.transpose(x0, [0, 2, -2, 1]))
    with pytest.raises(TVMError):
        bb.normalize(relax.op.transpose(x1, [0, 2, 2, 1]))
    with pytest.raises(TVMError):
        bb.normalize(relax.op.transpose(x1, [0, 2, -2, 1]))


def test_transpose_infer_struct_info_wrong_input_type():
    bb = relax.BlockBuilder()
    x0 = relax.Var("x", relax.ShapeStructInfo((1, 2, 3, 4)))
    x1 = relax.Var("x", relax.FuncStructInfo([], R.Tensor((1, 2, 3, 4), "float32")))

    with pytest.raises(TVMError):
        bb.normalize(relax.op.transpose(x0))
    with pytest.raises(TVMError):
        bb.normalize(relax.op.transpose(x1))
