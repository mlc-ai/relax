...
def test_matmul():
            ...
            with T.block("matmul_o"):
                ...
                for ax0, ax1 in T.grid(32, 4):
                    with T.block("A_shared"):
                        ...
                        T.reads(A_pad[v_i0_o * 32 + v0, v1])
                        T.writes(A_shared[v0, v1])
                        A_shared[v0, v1] = A_pad[v_i0_o * 32 + v0, v1]
                for i0_1, i1_1, k in T.grid(32, 32, 4):
                    with T.block("matmul"):
                        v_i0_i, v_i1_i, v_k_i = T.axis.remap("SSR", [i0_1, i1_1, k])
                        T.reads(
                            A_shared[v_i0_o * 32 + v_i0_i, v_k_i],
            ...
            with T.block("matmul_o"):
                ...
                for ax0, ax1 in T.grid(32, 4):
                    with T.block("A_shared"):
                        ...
                        T.writes(A_shared[v0, v1])
                        A_shared[v0, v1] = T.if_then_else(
                            v_i0_o * 32 + v0 < n, A[v_i0_o * 32 + v0, v1], T.float32(0)
                        )
                for i0_1, i1_1, k in T.grid(32, 32, 4):
                    with T.block("matmul"):
                        v_i0_i, v_i1_i, v_k_i = T.axis.remap("SSR", [i0_1, i1_1, k])
    ...
    b1 = sch.get_block(name="A_pad", func_name="main")
...
def test_norm_s3():
    ...
    class ModAfterS2:
        ...
        def main(
            ...
            # pad A
            for bsz, i, k in T.grid(
                T.int64(1), (n + T.int64(31)) // T.int64(32) * T.int64(32), T.int64(4096)
            ):
                with T.block("A_pad"):
                    v_bsz, v_i, v_k = T.axis.remap("SSS", [bsz, i, k])
                    T.reads(A[v_bsz, v_i, v_k])
                    T.writes(A_pad[v_bsz, v_i, v_k])
                    A_pad[v_bsz, v_i, v_k] = T.if_then_else(
                        v_i < n, A[v_bsz, v_i, v_k], T.float32(0)
                    )
                ...
                with T.block("compute_o"):
                    ...
                    for bsz, i_1, k in T.grid(T.int64(1), T.int64(32), T.int64(4096)):
                        with T.block("compute"):
                            ...
                            T.reads(A_pad[v_bsz, v_i_o * T.int64(32) + v_i_i, v_k_i])
                            T.writes(sq_sum_pad_local[v_i_i])
                            with T.init():
                                sq_sum_pad_local[v_i_i] = T.float32(0)
                            sq_sum_pad_local[v_i_i] = (
                                sq_sum_pad_local[v_i_i]
                                + A_pad[v_bsz, v_i_o * T.int64(32) + v_i_i, v_k_i]
                                * A_pad[v_bsz, v_i_o * T.int64(32) + v_i_i, v_k_i]
                            )
    ...
    sch = tir.Schedule(ModAfterS2, debug_mask="all")
    sch.compute_inline(sch.get_block("A_pad"))
    tvm.ir.assert_structural_equal(sch.mod["main"], ModAfterS3["main"])
