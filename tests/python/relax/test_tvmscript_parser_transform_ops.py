...
def test_reshape():
    ...
    with bb.function("foo", [x]):
        gv = bb.emit(relax.op.reshape(x, new_shape=(8, 3)))
        bb.emit_func_output(gv)
...
def test_reshape_infer_dim():
    ...
    bb = relax.BlockBuilder()
    with bb.function("foo", [x]):
        gv = bb.emit(relax.op.reshape(x, new_shape=(8, -1, 3)))
        bb.emit_func_output(gv)
...
def test_transpose():
    ...
    bb = relax.BlockBuilder()
    with bb.function("foo", [x]):
        gv = bb.emit(relax.op.transpose(x, axes=[1, -1, 2, -4]))
        bb.emit_func_output(gv)
...
def test_transpose_none_arg():
    ...
    def foo(x: R.Tensor((1, 2, 3, 4), "float32")) -> R.Tensor((4, 3, 2, 1), "float32"):
        gv: R.Tensor((4, 3, 2, 1), "float32") = R.transpose(x)
        return gv

    x = relax.Var("x", R.Tensor((1, 2, 3, 4), "float32"))
    bb = relax.BlockBuilder()
    with bb.function("foo", [x]):
        gv = bb.emit(relax.op.transpose(x))
        bb.emit_func_output(gv)
