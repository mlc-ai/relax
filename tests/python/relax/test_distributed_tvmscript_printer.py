...
def test_dtensor_struct_info():
    ...
    obj0 = DTensorStructInfo(tensor_sinfo1, DeviceMesh((2, 2), Range(0, 4)), Placement("S[1], R"))
    assert (
        obj0.__str__()
        == """R.DTensor((32, 32), "float32", I.device_mesh((2, 2), R.Range(0, 4)), "S[1], R")"""
    )

    obj1 = DTensorStructInfo(tensor_sinfo2, DeviceMesh((2, 2), Range(0, 4)), Placement("S[1], R"))
    assert (
        obj1.__str__()
...
@R.function
def main(x: R.DTensor((128, 128), "float32", I.device_mesh((2, 2), R.Range(0, 4)), "S[0], R")) -> R.DTensor((128, 128), "float32", I.device_mesh((2, 2), R.Range(0, 4)), "S[0], R"):
    gv0 = R.dist.call_tir(tir_func, (x,), out_sinfo=R.DTensor((128, 128), "float32", I.device_mesh((2, 2), R.Range(0, 4)), "S[0], R"))
    return gv0
            """,
    )
...
class Module:
    I.module_attrs({"device_num": 10})
    I.module_global_infos({"mesh": [I.device_mesh((2, 2), I.Range(0, 4)), I.device_mesh((1,), I.Range(4, 5))]})
    @T.prim_func
    ...
    def foo(x: R.DTensor((128, 128), "float32", "mesh[0]", "S[0], R")) -> R.DTensor((128, 128), "float32", "mesh[0]", "S[0], R"):
        gv0 = R.dist.call_tir(tir_func, (x,), out_sinfo=R.DTensor((128, 128), "float32", "mesh[0]", "S[0], R"))
        return gv0
    """,
    )
