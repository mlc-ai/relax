...
# Todo(ruihang): test symbolic shape.
...
def test_exp():
    ...
    class Expected:
        @R.function
        def expected(x: R.Tensor((2, 3), dtype="float32")) -> R.Tensor((2, 3), dtype="float32"):
        ...
        def exp(
            ...
            for i0, i1 in T.grid(T.int64(2), T.int64(3)):
                with T.block("compute"):
                    ...
                    T.reads(rxplaceholder[i0_1, i1_1])
                    T.writes(compute[i0_1, i1_1])
                    compute[i0_1, i1_1] = T.exp(rxplaceholder[i0_1, i1_1], dtype="float32")

    mod = OperatorLegalizer(Exp).transform()
    tvm.ir.assert_structural_equal(mod, Expected)
...
def test_permute_dims():
    ...
    class Expected:
        ...
        def transpose(
            rxplaceholder: T.Buffer[(T.int64(1), T.int64(2), T.int64(3), T.int64(4)), "float32"],
            T_permute_dims: T.Buffer[(T.int64(2), T.int64(4), T.int64(3), T.int64(1)), "float32"],
        ):
            T.func_attr({"tir.noalias": True})
            for i0, i1, i2, i3 in T.grid(T.int64(2), T.int64(4), T.int64(3), T.int64(1)):
                with T.block("T_permute_dims"):
                    ax0, ax1, ax2, ax3 = T.axis.remap("SSSS", [i0, i1, i2, i3])
                    T.reads(rxplaceholder[ax3, ax0, ax2, ax1])
                    T.writes(T_permute_dims[ax0, ax1, ax2, ax3])
                    T_permute_dims[ax0, ax1, ax2, ax3] = rxplaceholder[ax3, ax0, ax2, ax1]

    mod = OperatorLegalizer(PermuteDims).transform()
    tvm.ir.assert_structural_equal(mod, Expected)
...
def test_concat():
    ...
    mod = OperatorLegalizer(Concat).transform()
    tvm.ir.assert_structural_equal(mod, Expected)
...
def test_concat_input_tuple_var():
    ...
    class Concat:
        @R.function
        def main(
            x: R.Tuple(R.Tensor((3, 4), "float32"), R.Tensor((3, 5), "float32"))
        ) -> R.Tensor((3, 9), "float32"):
            gv: R.Tensor((3, 9), "float32") = R.concat(x, axis=1)
            return gv
    ...
    class Expected:
        @R.function
        def main(
            x: R.Tuple(R.Tensor((3, 4), dtype="float32"), R.Tensor((3, 5), dtype="float32"))
        ) -> R.Tensor((3, 9), dtype="float32"):
            gv: R.Tensor((3, 4), dtype="float32") = x[0]
            gv1: R.Tensor((3, 5), dtype="float32") = x[1]
            gv2 = R.call_tir(concatenate, (gv, gv1), (3, 9), dtype="float32")
            return gv2

        @T.prim_func
        def concatenate(
            rxplaceholder: T.Buffer[(T.int64(3), T.int64(4)), "float32"],
            rxplaceholder_1: T.Buffer[(T.int64(3), T.int64(5)), "float32"],
            ...
            for i0, i1 in T.grid(T.int64(3), T.int64(9)):
                with T.block("T_concat"):
                    ...
                    T_concat[ax0, ax1] = T.if_then_else(
                        T.int64(4) <= ax1,
                        rxplaceholder_1[ax0, ax1 - T.int64(4)],
...
@pytest.mark.skip("Cumsum is not introduced yet")
def test_cumsum():
    @I.ir_module
    class Cumsum:
        @R.function
...
def test_tril():
    ...
    class Trilu:
        @R.function
        def main(x: R.Tensor((2, 3, 4), "float32")) -> R.Tensor(None, "float32", ndim=3):
            gv: R.Tensor((2, 3, 4), "float32") = R.tril(x, k=0)
            return gv
    ...
    class Expected:
        ...
        def trilu(
            rxplaceholder: T.Buffer[(T.int64(2), T.int64(3), T.int64(4)), "float32"],
            tril: T.Buffer[(T.int64(2), T.int64(3), T.int64(4)), "float32"],
        ):
            T.func_attr({"tir.noalias": True})
            for i0, i1, i2 in T.grid(T.int64(2), T.int64(3), T.int64(4)):
                with T.block("trilu"):
                    ...
                    T.writes(tril[i0_1, i1_1, i2_1])
                    tril[i0_1, i1_1, i2_1] = T.Select(
...
def test_triu():
    ...
    class Trilu:
        @R.function
        def main(x: R.Tensor((2, 3, 4), "float32")) -> R.Tensor(None, "float32", ndim=3):
            gv: R.Tensor((2, 3, 4), "float32") = R.triu(x, k=2)
            return gv
            ...
            for i0, i1, i2 in T.grid(T.int64(2), T.int64(3), T.int64(4)):
                with T.block("trilu"):
                    ...
                    trilu[i0_1, i1_1, i2_1] = T.Select(
                        i1_1 <= i2_1 - T.int64(2), rxplaceholder[i0_1, i1_1, i2_1], T.float32(0)
                    )
...
def test_astype():
    ...
    mod = OperatorLegalizer(Astype).transform()
    tvm.ir.assert_structural_equal(mod, Expected)
...
def test_take():
    ...
    class Take:
        @R.function
        def main(
            x: R.Tensor((2, 3, 4), "float32"), indices: R.Tensor((4,), "int64")
    ...
    class Expected:
        @R.function
        def main(
            x: R.Tensor((2, 3, 4), dtype="float32"), indices: R.Tensor((4,), dtype="int64")
        ) -> R.Tensor(dtype="float32", ndim=5):
            gv = R.call_tir(take, (x, indices), (2, 4, 4), dtype="float32")
            return gv
        ...
        def take(
            ...
            T_take: T.Buffer[(T.int64(2), T.int64(4), T.int64(4)), "float32"],
        ):
            T.func_attr({"tir.noalias": True})
    ...
    class Full:
        ...
        def main(v: R.Tensor((), "int32")) -> R.Tensor(None, "float32", ndim=2):
            gv: R.Tensor((T.int64(2), T.int64(3)), "float32") = R.full(
                (T.int64(2), T.int64(3)), v, dtype="float32"
            )
            return gv
...
def test_full_like():
    ...
    class Expected:
        @R.function
        def main(
            x: R.Tensor((2, 3), dtype="float32"), y: R.Tensor((), dtype="float32")
        ) -> R.Tensor((2, 3), dtype="float32"):
            gv = R.call_tir(full, (y,), (2, 3), dtype="float32")
            return gv

        @T.prim_func
        def full(
            rxplaceholder: T.Buffer[(), "float32"],
            T_full: T.Buffer[(T.int64(2), T.int64(3)), "float32"],
        ):
            T.func_attr({"tir.noalias": True})
            for i0, i1 in T.grid(T.int64(2), T.int64(3)):
...
def test_ones_like():
    ...
    class Expected:
        ...
        def main(x: R.Tensor((2, 3), dtype="float32")) -> R.Tensor(None, dtype="float32", ndim=2):
            gv = R.call_tir(full, (), (2, 3), dtype="float32")
            return gv
        ...
        def full(
            ...
            for i0, i1 in T.grid(T.int64(2), T.int64(3)):
                with T.block("T_full_like"):
                    ...
                    T_full_like[ax0, ax1] = T.float32(1)
...
def test_zeros_like():
    ...
    class Expected:
        ...
        def main(x: R.Tensor((2, 3), dtype="float32")) -> R.Tensor((2, 3), dtype="float32"):
            gv = R.call_tir(full, R.tuple(), (2, 3), dtype="float32")
            return gv
            ...
            for i0, i1, i2 in T.grid(T.int64(1), T.int64(3), T.int64(2)):
                with T.block("rxplaceholder_red"):
                    ...
                    rxplaceholder_red[ax0, ax1] = (
                        rxplaceholder_red[ax0, ax1] + rxplaceholder[k0, ax1]
                    )
...
def test_log_softmax():
    ...
    class Expected:
        @R.function
        def main(
            x: R.Tensor((2, 3, 16, 32), dtype="float32")
        ) -> R.Tensor((2, 3, 16, 32), dtype="float32"):
        ...
        def log_softmax(
            rxplaceholder: T.Buffer[(T.int64(2), T.int64(3), T.int64(16), T.int64(32)), "float32"],
            compute: T.Buffer[(T.int64(2), T.int64(3), T.int64(16), T.int64(32)), "float32"],
        ):
            T.func_attr({"tir.noalias": True})
            T_softmax_maxelem = T.alloc_buffer(
                [T.int64(2), T.int64(3), T.int64(32)], dtype="float32"
            ...
            for i0, i1, i2, i3 in T.grid(T.int64(2), T.int64(3), T.int64(32), T.int64(16)):
                with T.block("compute"):
                    ...
                    compute_1[i0_2, i1_2, i2_2] = compute_1[i0_2, i1_2, i2_2] + T.exp(
                        rxplaceholder[i0_2, i1_2, k, i2_2] - T_softmax_maxelem[i0_2, i1_2, i2_2],
                        dtype="float32",
                    )
            for i0_3, i1_3, i2_3, i3 in T.grid(T.int64(2), T.int64(3), T.int64(16), T.int64(32)):
                with T.block("compute_1"):
                    ...
                    T.reads(
                        rxplaceholder[i0_4, i1_4, i2_4, i3_1],
                        T_softmax_maxelem[i0_4, i1_4, i3_1],
                        compute_1[i0_4, i1_4, i3_1],
                    )
                    T.writes(compute[i0_4, i1_4, i2_4, i3_1])
                    T.block_attr({"axis": 2})
                    compute[i0_4, i1_4, i2_4, i3_1] = (
                        rxplaceholder[i0_4, i1_4, i2_4, i3_1]
                        - T_softmax_maxelem[i0_4, i1_4, i3_1]
                        - T.log(compute_1[i0_4, i1_4, i3_1], dtype="float32")
                    )

    mod = OperatorLegalizer(LogSoftmax).transform()
    tvm.ir.assert_structural_equal(mod, Expected)
...
def test_cross_entropy_without_logits():
    ...
    class Expected:
        @R.function
        def main(
            x: R.Tensor((3,), dtype="float32"), y: R.Tensor((3,), dtype="float32")
    ...
    mod = OperatorLegalizer(CrossEntropyWithoutLogits).transform()
    tvm.ir.assert_structural_equal(mod, Expected)
...
def test_cross_entropy_with_logits():
    ...
    mod = OperatorLegalizer(CrossEntropyWithLogits).transform()
    tvm.ir.assert_structural_equal(mod, Expected)
