...
def te_reshape():
    ...
    two = tir.IntImm("int64", 2)
    four = tir.IntImm("int64", 4)
    A = te.placeholder((two, four), name="A")
    B = te.compute((4, 2), lambda x, y: A[(x * two + y) // four, (x * two + y) % four], name="B")
    return [A, B]
...
def tir_reshape(
    ...
    B: T.Buffer[(T.int64(4), T.int64(2)), "float32"],
):
    T.func_attr({"global_symbol": "main", "tir.noalias": True})
    for i0, i1 in T.grid(T.int64(4), T.int64(2)):
        with T.block("B"):
            x, y = T.axis.remap("SS", [i0, i1])
            T.reads(A[(x * T.int64(2) + y) // T.int64(4), (x * T.int64(2) + y) % T.int64(4)])
            T.writes(B[x, y])
            B[x, y] = A[(x * T.int64(2) + y) // T.int64(4), (x * T.int64(2) + y) % T.int64(4)]
