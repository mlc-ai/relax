...
def create_model():
    ...
    with bb.function("main", [x]):
        with bb.dataflow():
            ...
            lv3=bb.emit(relax.op.max_pool2d(lv2, (2, 2)))
            lv4=bb.emit(relax.op.flatten(lv3))
            lv5=bb.emit(relax.op.dense(lv4, linear0_weight))
            lv6=bb.emit(relax.op.add(lv5, linear0_bias))
            lv7=bb.emit(relax.op.relu(lv6))
            lv8=bb.emit(relax.op.dense(lv7, linear1_weight))
            lv9=bb.emit(relax.op.add(lv8, linear1_bias))
            lv10=bb.emit(relax.op.softmax(lv9))
