...
Expr conv2d_backward_weight(Expr output_grad, Expr data, Expr weight, Array<IntImm> strides,
                            Array<IntImm> padding, Array<IntImm> dilation, int groups,
  ...
  const Op& op = Op::Get("relax.grad.conv2d_backward_weight");
  return Call(op, {std::move(output_grad), std::move(data), std::move(weight)}, Attrs(attrs), {});
}

TVM_REGISTER_GLOBAL("relax.op.grad.conv2d_backward_weight").set_body_typed(conv2d_backward_weight);

StructInfo InferStructInfoConv2dBackwardWeight(const Call& call, const BlockBuilder& ctx) {
  return GetStructInfo(call->args[2]);
}

TVM_REGISTER_OP("relax.grad.conv2d_backward_weight")
    .set_num_inputs(3)
    .add_argument("output_grad", "Tensor", "The output gradient.")
