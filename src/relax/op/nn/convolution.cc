...
Expr conv2d_transpose(Expr data, Expr weight, Array<IntImm> strides, Array<IntImm> padding,
  ...
  attrs->output_padding = ConvertPrimExprToInt64(output_padding);
...
StructInfo InferStructInfoConv2dTranspose(const Call& call, const BlockBuilder& ctx) {
  ...
  if (analyzer->CanProve(attrs->output_padding[0] >= attrs->strides[0] ||
  ...
  } else if (!analyzer->CanProve(attrs->output_padding[0] < attrs->strides[0] &&
                                 attrs->output_padding[1] < attrs->strides[1])) {
