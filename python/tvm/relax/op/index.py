...
def take(data: Expr, indices: Expr, axis: Optional[int] = None) -> Expr:
    ...
    axis : Optional[int]
        ...
        If it is none, the input data is required to be one-dimensional.
    ...
    ret : relax.Expr
        The taken result.
    """
    return _ffi_api.take(data, indices, axis)  # type: ignore
...
def strided_slice(
    ...
    same length as `axes`.
    """
    return _ffi_api.strided_slice(data, axes, begin, end, strides)  # type: ignore
